...
| Vitals chart      | Built  | Pattern   | Track chart, Toggle group, Dropdown menu, Tooltip, Button | Vital signs on stacked tracks over a linear time axis, with bands from a schema, span presets, a track filter and three y-range modes. The Observation table is its equivalent without the chart.          |
