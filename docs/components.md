...
| Track chart   | Built  | None                             | Tracks of values on one shared axis, with bands, a crosshair and a tooltip. The clinical charts are built on it. Drawn in SVG by Cadence with no charting library, so its time axis falls on the local hour in a stated time zone: decision 0015. |
...
| Calendar    | Built  | react-day-picker, Button                                         | A month of days to choose from. Its own entry point, with react-day-picker 10 as an optional peer (decision 0014): most consumers will not need a calendar, and an entry point can join the barrel later without breaking anyone, where the reverse would. |
| Date picker | Built  | Calendar, Popover, Input, Button                                 | A date typed or chosen from a calendar, and a time where one is wanted. shadcn's date picker with an input, whose typed dates Cadence reads in the locale's order.                                                                                         |
| Toast       | Built  | Base UI Toast, Button                                            | A short message that something happened, such as a record saved. Never the only place an error is shown, because it goes away.                                                                                                                             |
...
| List and detail   | Built  | Button, in an Application shell | A list beside the record it selects, which is how most clinical work is laid out. |
...
| ----------------- | ------ | --------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Observation table | Built  | Pattern   | Table                                             | A flowsheet: one row per series, one column per round of readings, with each value's band and its change, and the source's own interpretation read out beside the band.                                    |
| Vitals chart      | Built  | Pattern   | Track chart, Tabs, Dropdown menu, Tooltip, Button | Vital signs on stacked tracks over a linear time axis, with bands from a schema, span presets, a track filter and three y-range modes. The Observation table is its equivalent without the chart.          |
| Medication card   | Built  | Composite | Card, Badge, Collapsible, Button                  | One medicine: name, dose, route, frequency and status, expanding to its dose history and when it was last given. For an order with its administrations, for a medication list, and in words for a patient. |
