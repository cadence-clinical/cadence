...
5. **The theme comes from npm.** A `registry:theme` item imports `@cadence-clinical/tokens/theme.css` and overwrites the stock `:root` and `.dark` variables. Tokens stay versioned and contrast-tested, and are not a file a consumer can edit into a failing state.

6. **The website serves the registry through a route handler**, at `https://www.cadenceclinical.dev/r/{name}.json`, under the `@cadence` namespace. The site build runs `shadcn build` into a directory that is not public, and the handler returns the item. It starts open. Because it is a handler and not static files, a token check can be added later without changing a URL. The handler stamps each item's `meta` with the package version and the commit it was built from.
...
- The scope is `ui` now and `clinical` later. `core`, `fhir` and Region packages are npm only.
