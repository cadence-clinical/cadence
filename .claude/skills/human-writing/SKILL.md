---
name: human-writing
description: Edit, review, audit, or draft standalone prose while preserving the writer's meaning and voice. Use for the Cadence Clinical landing page and site copy, announcements, blog posts, READMEs' introductory prose, emails and other dedicated writing when the user wants clearer, more direct, more natural, or less AI-patterned language, or asks whether a draft contains AI-slop patterns. For reference and instructional documentation use technical-writing instead. Do not use for routine conversational replies or code unless the user asks to edit its prose.
---
...
Apply these rules when writing for the Cadence Clinical website or announcements:

- The readers are clinicians, clinical informaticians and the engineers who build for them. They are sceptical of marketing and quick to spot an overclaim. Say what Cadence does and let them judge.
- Never claim or imply that Cadence is validated, certified, safe or approved, or that it improves outcomes. State a component's grade, not an impression of its quality. Cadence is a presentation layer and is not a medical device.
- Do not state planned work as available. If the packages are not on npm, the copy says so.
- A clinical fact, threshold or guideline requirement appears only with its published source.
- Use Australian English (colour, licence as a noun, organisation).
- Use synthetic examples only. Never a real patient's details.
- For reference and instructional pages under `apps/site/content/docs`, use the `technical-writing` skill instead.
...
Adapted for Cadence Clinical from the `human-writing` skill in [vercel/eve](https://github.com/vercel/eve) at commit `6fbff53c9`, Copyright 2026 Vercel, Inc. and contributors, licensed under the Apache License, Version 2.0. Upstream has since replaced this skill with `technical-writing`. Changes: the description was re-scoped, and the section on the upstream project's technical documentation was replaced with rules for Cadence site copy. See [LICENSE-APACHE](../LICENSE-APACHE).
