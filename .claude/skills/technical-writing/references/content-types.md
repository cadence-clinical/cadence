# Cadence documentation content types
...
- Keep the top-level sidebar lean. `apps/site/content/docs/meta.json` moves from orientation (Introduction, Installation, Theming) through Principles to the component reference (Primitives, then Clinical).
- Nest a page under the capability that owns it when the page only matters after the reader knows the parent (for example, a FHIR transform under the clinical component it feeds).
...
- When moving a published route, update authored links and add permanent redirects for every old HTML and Markdown URL.
